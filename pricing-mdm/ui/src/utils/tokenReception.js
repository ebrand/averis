...
export const processIncomingAuth = () => {
  ...
  console.log('DEBUG processIncomingAuth:', { 
    authToken: authToken ? 'Present' : 'Not found',
    authSession: authSession ? 'Present' : 'Not found', 
    source,
  ...
  if (authSession) {
    try {
      ...
      console.log('DEBUG Session data received:', { 
        ...
        hasDbUser: !!parsedSession.dbUser,
        userRoles: parsedSession.user?.roles || [],
        dbUserRoles: parsedSession.dbUser?.roles || [],
        stytchUserDetails: parsedSession.user ? {
          userId: parsedSession.user.user_id,
          email: parsedSession.user.emails?.[0]?.email,
          roles: parsedSession.user.roles
        } : null,
        dbUserDetails: parsedSession.dbUser ? {
          id: parsedSession.dbUser.id,
          email: parsedSession.dbUser.email,
          roles: parsedSession.dbUser.roles
        } : null
      })
