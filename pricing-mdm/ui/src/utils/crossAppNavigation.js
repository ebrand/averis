/**
 * Cross-App Navigation Utility for Pricing MDM
 * Handles navigation between Dashboard, Product MDM, and Pricing MDM with token passing
 */
 ...
 * @param {string} appKey - The target application key (dashboard, product, customer, oms, erp)
...
export const navigateToApp = (appKey, path = '/', openInNewTab = false) => {
  ...
  targetUrl.searchParams.set('source', 'pricing-mdm')
...
export const navigateToDashboard = (path = '/', openInNewTab = false) => {
  ...
  try {
    ...
    if (token) {
      targetUrl.searchParams.set('auth_token', token)
      targetUrl.searchParams.set('source', 'pricing-mdm')
      console.log('Pricing MDM: Navigating to Dashboard with authentication token')
    }
    ...
    console.error('Pricing MDM: Failed to navigate to Dashboard:', error)
...
/**
 * Navigate to Product MDM with current authentication token
 * @param {string} path - Path to navigate to in Product MDM (default: '/')
 * @param {boolean} openInNewTab - Whether to open in a new tab
 */
export const navigateToProductMdm = (path = '/', openInNewTab = false) => {
  const productUrl = 'http://localhost:3001'
  const token = localStorage.getItem('stytch_session_token')
  
  try {
    const targetUrl = new URL(path, productUrl)
    
    // Add authentication token if available
    if (token) {
      targetUrl.searchParams.set('auth_token', token)
      targetUrl.searchParams.set('source', 'pricing-mdm')
      console.log('Pricing MDM: Navigating to Product MDM with authentication token')
    }
    ...
    console.error('Pricing MDM: Failed to navigate to Product MDM:', error)
    // Fallback navigation without token
    if (openInNewTab) {
...
export const navigateToCustomerMdm = (path = '/customers', openInNewTab = false) => {
  ...
  try {
    ...
    if (token) {
      targetUrl.searchParams.set('auth_token', token)
      targetUrl.searchParams.set('source', 'pricing-mdm')
      console.log('Pricing MDM: Navigating to Customer MDM with authentication token')
    }
    ...
    console.error('Pricing MDM: Failed to navigate to Customer MDM:', error)
    // Fallback navigation without token
    if (openInNewTab) {
...
/**
 * Handle navigation with keyboard modifier detection for Product MDM
 * @param {Event} event - Click event
 * @param {string} path - Path to navigate to
 */
export const handleProductMdmNavigation = (event, path = '/') => {
  const openInNewTab = event.ctrlKey || event.metaKey
  event.preventDefault()
  navigateToProductMdm(path, openInNewTab)
}
