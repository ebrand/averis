...
export const AuthProvider = ({ children }) => {
  ...
  const buildStaticRoleGroups = (userRoles) => {
    ...
    // Role group definitions for pricing-mdm
    const roleGroupDefinitions = {
      CatalogMaintainer: {
        name: 'Catalog Maintainer',
        icon: '📖',
        color: 'bg-blue-100 text-blue-800',
        description: 'Catalog management and approval across regions',
        roles: {
          'catalog_amer': 'catalog_amer_approve',
          'catalog_emea': 'catalog_emea_approve'
        }
      },
      PricingMaintainer: {
        name: 'Pricing Maintainer',
        icon: '💰',
        color: 'bg-green-100 text-green-800',
        description: 'Pricing management and approval across regions',
        roles: {
          'pricing_amer': 'pricing_amer_approve',
          'pricing_emea': 'pricing_emea_approve'
        }
      }
    }
    ...
    Object.entries(roleGroupDefinitions).forEach(([groupKey, groupDef]) => {
      ...
      // Check each base role and its corresponding approval role
      Object.entries(groupDef.roles).forEach(([baseRole, approveRole]) => {
        if (userRoles.includes(baseRole)) constituentRoles.push(baseRole)
        if (userRoles.includes(approveRole)) constituentRoles.push(approveRole)
      })
      
      if (constituentRoles.length > 0) {
  ...
  const syncUserWithDatabase = async (stytchUser) => {
    ...
    try {
      ...
      if (searchResult.users && searchResult.users.length > 0) {
        ...
        console.log('DEBUG AuthContext - Existing user found:', dbUserRecord.email, 'with roles:', dbUserRecord.roles)
        
        // Check if existing user has catalog roles - if not, we might need to update them
        const hasCatalogRoles = dbUserRecord.roles?.some(role => role.startsWith('catalog_'))
        const stytchCatalogRoles = stytchUser.roles?.filter(role => role.startsWith('catalog_')) || []
        console.log('DEBUG AuthContext - Catalog roles check:', { hasCatalogRoles, stytchCatalogRoles })
        ...
        const validRoles = userRoles.filter(role => 
          role.startsWith('catalog_') || ['admin', 'user_admin', 'averis_pricing'].includes(role)
        )
        
        console.log('DEBUG AuthContext - New user role validation:', {
          allUserRoles: userRoles,
          hasAdminRole,
          validRoles,
          adminInValidRoles: validRoles.includes('admin'),
          catalogRoles: userRoles.filter(r => r.startsWith('catalog_'))
        })
        
        console.log('DEBUG AuthContext - New user access check:', { 
          userRoles, 
          hasAdminRole, 
          validRoles,
          catalogRoles: userRoles.filter(r => r.startsWith('catalog_'))
        ...
        // Admin users always get access, regardless of specific pricing roles
        if (hasAdminRole || validRoles.length > 0) {
          console.log('Creating new user in database with roles:', hasAdminRole ? ['admin', ...validRoles] : validRoles)
          
          // Create new user record
          // For admin users, ensure admin role is preserved along with all valid roles
          let finalRoles = [...new Set(validRoles)] // Remove duplicates
          if (hasAdminRole && !finalRoles.includes('admin')) {
            finalRoles.unshift('admin') // Add admin at the beginning if not already included
          }
          
          console.log('DEBUG AuthContext - Creating user with final roles:', finalRoles)
          ...
          console.warn('User has no valid roles for Pricing MDM:', userRoles)
          console.warn('Required: admin, user_admin, averis_pricing, or catalog_* roles')
          throw new Error('User does not have the required pricing roles to access Pricing MDM')
        }
      }
  ...
  const initializeAuth = async () => {
    try {
      ...
      console.log('Pricing MDM AuthContext: initializeAuth starting')
      console.log('Pricing MDM AuthContext: Logout URL parameter:', logoutParam)
      
      // If logout parameter exists, FORCE logout immediately regardless of other state
      if (logoutParam) {
        console.log('Pricing MDM AuthContext: FORCED LOGOUT - Logout parameter detected, clearing all session data')
        ...
        // Clean up the URL
        console.log('Pricing MDM AuthContext: Cleaning logout parameter from URL')
        const cleanUrl = window.location.origin + window.location.pathname
        window.history.replaceState({}, document.title, cleanUrl)
        
        setIsLoading(false)
        console.log('Pricing MDM AuthContext: FORCED LOGOUT COMPLETE - staying logged out')
        return
      }
      ...
      console.log('Pricing MDM AuthContext: Comprehensive logout detection:', { 
        recentLogout, 
        forcedLogout, 
        permanentLogout,
      ...
      if (recentLogout || forcedLogout || permanentLogout) {
        console.log('Pricing MDM AuthContext: LOGOUT DETECTED - preventing authentication restoration')
        ...
        setIsLoading(false)
        console.log('Pricing MDM AuthContext: LOGOUT PREVENTION COMPLETE - staying logged out')
        return
      }
      ...
      if (token && storedSession) {
        try {
          ...
          console.log('DEBUG AuthContext - initializeAuth session data:', {
            hasUser: !!user,
            hasDbUser: !!dbUser,
          ...
          if (user) {
            ...
            // Build static role groups - prioritize fresh Stytch user roles over cached dbUser
            const rolesToUse = user?.roles && user.roles.length > 0 ? user.roles : (dbUser?.roles || [])
            console.log('DEBUG AuthContext - Role priority decision:', {
              stytchRoles: user?.roles || [],
              dbUserRoles: dbUser?.roles || [],
              rolesToUse,
              usingStytchRoles: user?.roles && user.roles.length > 0
            })
            
            if (rolesToUse.length > 0) {
              console.log('DEBUG AuthContext - Building static role groups from roles:', rolesToUse)
              const staticGroups = buildStaticRoleGroups(rolesToUse)
              setStaticRoleGroups(staticGroups)
            } else {
              console.log('DEBUG AuthContext - No roles found, cannot build static role groups')
            }
              ...
              if (!dbUser) {
                ...
                const fallbackDbUser = {
                  ...
                  roles: [
                    'admin',
                    'pricing_amer',
                    'pricing_amer_approve', 
                    'pricing_emea',
                    'pricing_emea_approve',
                    'catalog_amer',
                    'catalog_amer_approve',
                    'catalog_emea', 
                    'catalog_emea_approve'
                  ],
                  status: 'active'
                }
  ...
  const handleLogout = async () => {
    console.log('Pricing MDM AuthContext: handleLogout started - using centralized logout')
    try {
      ...
      console.log('Pricing MDM AuthContext: Local state cleared, initiating centralized logout')
      
      // Perform centralized logout across all applications
      await performCentralizedLogout()
      
    } catch (error) {
      console.error('Pricing MDM AuthContext: Centralized logout error:', error)
      // Fallback to Dashboard login if centralized logout fails
    ...
    try {
      ...
      if (dbUser) {
        const dbUpdates = {
          ...
          updatedBy: dbUser.id, // Set who is making the update
          // Can add more fields here as needed
        }
        
        const updateResult = await userUtils.updateUser(dbUser.id, dbUpdates)
        if (updateResult.error) {
          console.warn('Failed to update database user:', updateResult.error)
        } else {
