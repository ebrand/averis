...
const REGION_CONFIG = {
  amer: {
    ...
    color: 'blue',
    description: 'Manage pricing catalogs for the Americas region across different sales channels.'
  },
  emea: {
    code: 'EMEA',
    ...
    color: 'green',
    description: 'Manage pricing catalogs for the Europe, Middle East & Africa region across different sales channels.'
  },
  apj: {
    code: 'APJ',
    name: 'Asia Pacific & Japan',
    fullName: 'Asia Pacific & Japan',
    icon: GlobeAsiaAustraliaIcon,
    color: 'indigo',
    description: 'Manage pricing catalogs for the Asia Pacific & Japan region across different sales channels.'
  },
  la: {
    code: 'LA',
    name: 'Latin America',
    ...
    icon: GlobeAltIcon,
    color: 'red',
    description: 'Manage pricing catalogs for the Latin America region across different sales channels.'
  }
}

const RegionalCatalogsPage = () => {
  const { hasPermission, canViewCatalogs, canEditCatalogs, hasRegionAccess } = useRole()
  const navigate = useNavigate()
  const { region: regionParam } = useParams()
  
  // Get region configuration
  const regionConfig = REGION_CONFIG[regionParam?.toLowerCase()]
  
  // Redirect if invalid region
  useEffect(() => {
    if (!regionConfig) {
      navigate('/catalogs')
      return
    }
  }, [regionConfig, navigate])
  ...
  const loadCatalogs = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/catalogs?region=${regionConfig.code}`, {
        headers: {
  ...
  const loadCurrencies = async () => {
    try {
      const response = await fetch('/api/catalogs/currencies', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      })
  ...
  const handleChannelSelection = (channelId) => {
    ...
    if (selectedChannel) {
      // Generate default code and name based on region
      const channelCode = selectedChannel.code || selectedChannel.name.toUpperCase().replace(/\s+/g, '_')
      const defaultCode = `${regionConfig.code}_${channelCode}_`
      const defaultName = `${regionConfig.fullName} ${selectedChannel.name} Catalog`
  ...
  const handleCreateCatalog = async () => {
    try {
      // Get region and currency info
      const currentRegion = regions.find(r => r.code === regionConfig.code)
  ...
  // Return early if invalid region
  if (!regionConfig) {
    return null
  }

  if (!canViewCatalogs() || !hasRegionAccess(regionConfig.code)) {
    return (
      <div className="px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            You don't have permission to manage catalogs for {regionConfig.fullName}.
          </p>
        </div>
      </div>
    )
      ...
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <div className="flex items-center">
            <RegionIcon className={`h-8 w-8 text-${regionConfig.color}-600 mr-3`} />
            <div>
              <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">{regionConfig.fullName} Catalogs</h1>
        ...
        {canEditCatalogs() && (
          <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <button
              ...
              className={`inline-flex items-center justify-center rounded-md border border-transparent bg-${regionConfig.color}-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-${regionConfig.color}-700 focus:outline-none focus:ring-2 focus:ring-${regionConfig.color}-500 focus:ring-offset-2`}
            >
        ...
        <div className="flex-1">
          <div className="relative">
            ...
            <input
              ...
              className={`block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md leading-5 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-${regionConfig.color}-500 focus:border-${regionConfig.color}-500`}
            />
          </div>
        </div>
      </div>
      ...
      <div className="mt-8">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Get started by creating a new catalog for the {regionConfig.fullName} region.
            </p>
            {canEditCatalogs() && (
              <div className="mt-6">
                <button
                  type="button"
                  onClick={handleOpenCreateModal}
                  className={`inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-${regionConfig.color}-600 hover:bg-${regionConfig.color}-700`}
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <div className="flex-shrink-0">
                        <BuildingStorefrontIcon className={`h-10 w-10 text-${regionConfig.color}-500`} />
                      </div>
                      <div className="ml-4">
                        <div className="flex items-center">
                          ...
                          <span className={`ml-3 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            catalog.status === 'active' 
                              ? `bg-${regionConfig.color}-100 text-${regionConfig.color}-800` 
                    ...
                    <div className="flex items-center space-x-2">
                      <Link
                        to={`/catalogs/${regionParam}/${catalog.id}`}
                        className={`inline-flex items-center px-3 py-2 border border-gray-300 dark:border-gray-600 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-${regionConfig.color}-500`}
                      >
            ...
            <div className="mt-3">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className={`mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-${regionConfig.color}-500 focus:border-${regionConfig.color}-500`}
                ...
                <div className="relative">
                  ...
                  <div className="relative">
                    <button
                      ...
                      className={`mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-${regionConfig.color}-500 focus:border-${regionConfig.color}-500 text-left`}
                    >
                ...
                <div className="flex items-center">
                  <input
                    ...
                    className={`h-4 w-4 text-${regionConfig.color}-600 focus:ring-${regionConfig.color}-500 border-gray-300 dark:border-gray-600 rounded`}
                  />
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                <button
                  ...
                  className={`px-4 py-2 bg-${regionConfig.color}-600 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-${regionConfig.color}-700 disabled:opacity-50 disabled:cursor-not-allowed`}
                >
                  Update Catalog
                </button>
              </div>
                  ...
                  <>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCatalogForm({...catalogForm, code: e.target.value})}
                        className={`mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-${regionConfig.color}-500 focus:border-${regionConfig.color}-500`}
                        placeholder={`e.g., ${regionConfig.code}_DIRECT_`}
                        required
                      />
                      <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCatalogForm({...catalogForm, name: e.target.value})}
                        className={`mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-${regionConfig.color}-500 focus:border-${regionConfig.color}-500`}
                        placeholder={`e.g., ${regionConfig.fullName} Direct Sales Catalog`}
                        required
                      />
                      <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
...
export default RegionalCatalogsPage