...
class MessageLogService {
  constructor() {
    ...
    this.productStagingUrl = import.meta.env.VITE_PRODUCT_STAGING_API_BASE_URL || 'http://api.localhost/product-staging/api'
  ...
  async generateStagingActivity() {
    try {
      ...
      const stagingResponse = await fetch(`${this.productStagingUrl}/products?limit=20`)
