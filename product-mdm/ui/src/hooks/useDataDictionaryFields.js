  ...
  useEffect(() => {
    const fetchDataDictionary = async () => {
      try {
        ...
        // Clear cache and mark as currently fetching to get fresh data
        dataDictionaryCache = null
        cacheTimestamp = null
        isCurrentlyFetching = true
        console.log('🔄 Fetching fresh data dictionary from API (cache cleared)')
        
        const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || '/api'
        const response = await fetch(`${apiBaseUrl}/data-dictionary?schema=ProductMDM&_t=${Date.now()}`)
