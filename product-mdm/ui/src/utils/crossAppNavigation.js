/**
 * Cross-App Navigation Utility for Product MDM
 ...
 * @param {string} appKey - The target application key (dashboard, pricing, customer, oms, erp)
...
export const navigateToApp = (appKey, path = '/', openInNewTab = false) => {
  ...
  targetUrl.searchParams.set('source', 'product-mdm')
...
export const navigateToDashboard = (path = '/', openInNewTab = false) => {
  ...
  try {
    ...
    if (token) {
      targetUrl.searchParams.set('auth_token', token)
      targetUrl.searchParams.set('source', 'product-mdm')
      console.log('Product MDM: Navigating to Dashboard with authentication token')
    }
...
export const navigateToPricingMdm = (path = '/products', openInNewTab = false) => {
  ...
  try {
    ...
    if (token) {
      targetUrl.searchParams.set('auth_token', token)
      targetUrl.searchParams.set('source', 'product-mdm')
      console.log('Product MDM: Navigating to Pricing MDM with authentication token')
    }
...
export const navigateToCustomerMdm = (path = '/customers', openInNewTab = false) => {
  ...
  try {
    ...
    if (token) {
      targetUrl.searchParams.set('auth_token', token)
      targetUrl.searchParams.set('source', 'product-mdm')
      console.log('Product MDM: Navigating to Customer MDM with authentication token')
    }
