...
export const centralizedLogout = async () => {
  ...
  try {
    ...
    // Clear any profile pictures stored locally
...
export const isRecentLogout = () => {
  ...
  const isRecent = timeSinceLogout < 300000 // 5 minutes - much longer window for new browser sessions
  console.log('isRecentLogout check:', { logoutTimestamp, timeSinceLogout, isRecent })
  return isRecent
}
...
export const redirectToCentralLogin = () => {
  ...
  const redirectUrl = `http://localhost:3012/login?logout=${logoutTimestamp}`
...
export const performCentralizedLogout = async () => {
  ...
  console.log('Centralized logout completed, redirecting to login')
  
  // Call redirect immediately - no setTimeout to avoid cancellation issues
