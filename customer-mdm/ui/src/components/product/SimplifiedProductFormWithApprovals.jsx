...
const SimplifiedProductFormWithApprovals = ({ 
  ...
  // Load approval state from localStorage or API
  useEffect(() => {
    if (productId) {
      const savedApprovals = localStorage.getItem(`product_approvals_${productId}`)
      if (savedApprovals) {
        try {
  ...
  const saveApprovals = (newApprovals) => {
    ...
    if (productId) {
      localStorage.setItem(`product_approvals_${productId}`, JSON.stringify(newApprovals))
    }
    // Also update form data approvals field
    setFormData(prev => ({ ...prev, approvals: Object.entries(newApprovals).map(([key, value]) => ({ section: key, ...value })) }))
  }
  ...
  const handleApproval = (sectionKey, approved) => {
    const newApprovals = {
      ...approvals,
      [sectionKey]: {
        approved,
        approvedBy: approved ? userProfile?.email || 'Current User' : null,
  ...
  const handleLaunch = async () => {
    ...
    try {
      // Create launch data with active status
      const launchData = { ...formData, status: 'active' }
