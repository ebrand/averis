...
export const SideNavBar = ({ isOpen, onClose }) => {
  ...
  const getInitialExpandedSections = () => {
    ...
    // Default: always open Customers section for new users
  ...
  const buildNavigationItems = () => {
    ...
    items.push({
      ...
      children: [
        { name: 'All Customers', href: '/customers', icon: UsersIcon },
        { name: 'Prospects', href: '/customers/prospects', icon: UserPlusIcon },
        { name: 'Accounts', href: '/customers/accounts', icon: BuildingOfficeIcon }
  ...
  const navigation = [
    ...
    ...(canAccessAdminFeatures() ? [{
      ...
      children: [
        ...(isUserAdmin() || isAdmin() ? [
          { name: 'Staging View', href: '/customers/staging', icon: CircleStackIcon }
        ] : [])
      ]
    }] : [])
  ]
  ...
  const NavItem = ({ item, isChild = false, isExpanded, onToggle }) => {
    ...
    // Custom active check for "All Customers" - consider both "/" and "/customers" as active
    const isCustomActive = (href) => {
      if (href === '/customers') {
        return location.pathname === '/' || location.pathname === '/customers'
      }
      return false
    }
    ...
    if (hasChildren && !isChild) {
      ...
      return (
        <button
          ...
          className={`${baseClasses} w-full text-left text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-red-50 dark:hover:bg-gray-800`}
        >
    ...
    if (item.external) {
      return (
        <a
          ...
          className={`${baseClasses} text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-red-50 dark:hover:bg-gray-800`}
        >
    ...
    return (
      <NavLink
        ...
        className={({ isActive }) => {
          // Use custom active check for "All Customers", fallback to default for others
          const activeState = isCustomActive(item.href) || isActive
          return `${baseClasses} ${
            activeState
              ? isChild
                ? 'bg-red-50 dark:bg-red-900/20 border-r-2 border-red-700 text-red-700 dark:text-red-300'
                : 'bg-red-100 dark:bg-red-900/30 text-red-900 dark:text-red-200 shadow-sm'
              : 'text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-red-50 dark:hover:bg-gray-800'
          }`
        }}
      >
        {(!isChild || (isChild && item.icon)) && <item.icon className="mr-3 h-5 w-5 flex-shrink-0" />}
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="border-b border-gray-200 dark:border-gray-700 px-4 py-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">Customer relationship mgmt</p>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {platformApps.map((app, index) => {
                ...
                const gridClass = app.size === 'large' 
                  ? 'col-span-2 row-span-1' // Customer button spans 2 columns
