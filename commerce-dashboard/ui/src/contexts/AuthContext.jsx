...
export const AuthProvider = ({ children }) => {
  ...
  const initializeAuth = async () => {
    try {
      ...
      console.log('Dashboard AuthContext: initializeAuth starting')
      console.log('Dashboard AuthContext: Logout URL parameter:', logoutParam)
      
      // If logout parameter exists, FORCE logout immediately regardless of other state
      if (logoutParam) {
        console.log('Dashboard AuthContext: FORCED LOGOUT - Logout parameter detected, clearing all session data')
        ...
        // Clean up the URL
        console.log('Dashboard AuthContext: Cleaning logout parameter from URL')
        const cleanUrl = window.location.origin + window.location.pathname
        window.history.replaceState({}, document.title, cleanUrl)
        
        setIsLoading(false)
        console.log('Dashboard AuthContext: FORCED LOGOUT COMPLETE - staying logged out')
        return
      }
      ...
      console.log('Dashboard AuthContext: Comprehensive logout detection:', { 
        recentLogout, 
        forcedLogout, 
        permanentLogout,
      ...
      if (recentLogout || forcedLogout || permanentLogout) {
        console.log('Dashboard AuthContext: LOGOUT DETECTED - preventing authentication restoration')
        ...
        setIsLoading(false)
        console.log('Dashboard AuthContext: LOGOUT PREVENTION COMPLETE - staying logged out')
        return
      }
  ...
  const updateProfile = async (updates) => {
    try {
      ...
      if (dbUser) {
        // For now, update local state only since the User MDM API has database schema issues
        // TODO: Fix the User MDM API database schema for array fields
        updatedDbUser = {
          ...dbUser,
        ...
        console.log('Updating local user state (database update skipped due to API issues):', updatedDbUser)
        setDbUser(updatedDbUser)
        
        // TODO: Implement proper database update once User MDM API is fixed
        // const dbUpdates = {
        //   FirstName: updates.firstName !== undefined ? updates.firstName : dbUser.firstName,
        //   LastName: updates.lastName !== undefined ? updates.lastName : dbUser.lastName,
        //   Email: dbUser.email,
        //   StytchUserId: dbUser.stytchUserId,
        //   Roles: dbUser.roles || [],
        //   Status: dbUser.status || 'active',
        //   PreferencesJson: dbUser.preferencesJson || '{}'
        // }
