...
export const SideNavBar = ({ isOpen, onClose }) => {
  ...
  const getInitialExpandedSections = () => {
    ...
    // Default: always open Services section for new users
  ...
  const navigation = [
    {
      ...
      children: [
        ...
        { name: 'Real-time Metrics', href: '/analytics/realtime' },
        { name: 'System Logs', href: '/services/logs' }
      ]
    },
    ...(canAccessAdminFeatures() ? [{
      ...
      children: [
        ...
        { name: 'Product Data Dictionary', href: '/admin/data-dictionary', icon: BookOpenIcon },
  ...
  const NavItem = ({ item, isChild = false, isExpanded, onToggle }) => {
    ...
    // Handle external links (like Product to port 3001, Pricing to port 3003)
    if (item.external) {
      return (
        <a
          href={item.href}
          onClick={(e) => {
            e.preventDefault()
            // Use cross-app navigation utility
            if (item.appKey) {
              navigateToApp(item.appKey, '/')
            } else {
      ...
      <NavLink
        ...
        className={({ isActive }) =>
          `${baseClasses} ${
            isActive
              ? isChild
                ? 'bg-teal-50 dark:bg-teal-900/20 border-r-2 border-teal-500 text-teal-700 dark:text-teal-300'
                : 'bg-teal-100 dark:bg-teal-900/30 text-teal-900 dark:text-teal-200 shadow-sm'
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed left-0 z-50 h-full w-64 transform bg-white dark:bg-gray-800 shadow-lg transition-transform duration-300 ease-in-out lg:relative lg:top-0 lg:translate-x-0 lg:shadow-none lg:border-r lg:border-gray-200 dark:border-gray-700 ${
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="border-b border-gray-200 dark:border-gray-700 px-4 py-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">System monitoring dashboard</p>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {platformApps.map((app, index) => {
                ...
                const gridClass = app.size === 'large' 
                  ? 'col-span-2 row-span-1' // Home button spans 2 columns
