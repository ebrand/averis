...
import { navigateToApp } from '../../utils/crossAppNavigation'
import {
  HomeIcon,
  CubeIcon,
  ShoppingCartIcon,
...
export const SideNavBar = ({ isOpen, onClose }) => {
  ...
  const getInitialExpandedSections = () => {
    ...
    // Default: always open Orders section for new users  
    return { Orders: true }
  }
  ...
  const navigation = [
    {
      ...
      children: [
        ...
        { name: 'Shipments', href: '/orders/shipments' }
      ]
    },
    {
      name: 'Fulfillment',
      ...
      children: [
        { name: 'Pick Lists', href: '/fulfillment/pick-lists' },
        { name: 'Packing', href: '/fulfillment/packing' },
        { name: 'Shipping', href: '/fulfillment/shipping' },
        { name: 'Tracking', href: '/fulfillment/tracking' }
      ]
    },
    {
      name: 'Reports',
      ...
      children: [
        ...
        { name: 'Fulfillment Reports', href: '/reports/fulfillment' },
    ...
    ...(canAccessAdminFeatures() ? [{
      ...
      children: [
        // Data Dictionary and Users have been moved to the Dashboard app
        // Add other admin items here if needed in the future
      ]
    }] : [])
  ]
  ...
  const NavItem = ({ item, isChild = false, isExpanded, onToggle }) => {
    ...
    // Custom active check for "All Orders" - consider both "/" and "/orders" as active
    const isCustomActive = (href) => {
      if (href === '/orders') {
        return location.pathname === '/' || location.pathname === '/orders'
      }
      return false
    }
    ...
    // Handle external links (like Dashboard Overview to port 3012, Pricing to port 3003)
    if (item.external) {
      return (
        <a
          href={item.href}
    ...
    return (
      <NavLink
        ...
        className={({ isActive }) => {
          // Use custom active check for "All Products", fallback to default for others
          ...
          return `${baseClasses} ${
            activeState
              ? isChild
                ...
                : 'bg-blue-100 dark:bg-blue-900/30 text-blue-900 dark:text-blue-200 shadow-sm'
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="border-b border-gray-200 dark:border-gray-700 px-4 py-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">Order & fulfillment mgmt</p>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {platformApps.map((app, index) => {
                ...
                const gridClass = app.size === 'large' 
                  ? 'col-span-2 row-span-1' // OMS button spans 2 columns
