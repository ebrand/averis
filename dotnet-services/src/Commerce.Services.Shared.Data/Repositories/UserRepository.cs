...
/// Handles all database interactions for User entities with PostgreSQL
/// </summary>
public class UserRepository : IUserRepository
{
    ...
    public UserRepository(ProductMdmDbContext context, ILogger<UserRepository> logger)
    {
    ...
    public async Task<bool> UpdateLastLoginAsync(int id, DateTime lastLogin)
    {
        ...
        if (user == null)
        {
            _logger.LogWarning("User not found for last login update: {UserId}", id);
            return false;
        }
        ...
        _logger.LogDebug("Updated last login for user: {UserId}", id);
        return true;
    }
    ...
    public async Task<UserAnalytics> GetUserAnalyticsAsync()
    {
        ...
        _logger.LogInformation("Generated analytics for {TotalUsers} users", analytics.TotalUsers);
        return analytics;
    }
}