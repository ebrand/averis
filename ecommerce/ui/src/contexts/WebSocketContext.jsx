...
export const WebSocketProvider = ({ children }) => {
  ...
  const connect = () => {
    try {
      const wsUrl = `ws://localhost:8087/averis_ecomm-updates`
      ...
      wsRef.current.onopen = () => {
        console.log('🔌 E-commerce WebSocket connected')
        setIsConnected(true)
      ...
      wsRef.current.onmessage = (event) => {
        try {
          ...
          console.log('📨 E-commerce WebSocket message:', message)
      ...
      wsRef.current.onerror = (error) => {
        console.error('❌ E-commerce WebSocket error:', error)
        setConnectionStatus('error')
      }

      wsRef.current.onclose = (event) => {
        console.log('🔌 E-commerce WebSocket disconnected:', event.code, event.reason)
